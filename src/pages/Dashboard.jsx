...
//                   showDropdownEditors // ✅ Pass prop to enable dropdowns
//                 />
//               ))}
//             </div>
//           )}
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const unsubClients = onSnapshot(collection(db, "clients"), (snapshot) => {
      setClients(snapshot.empty ? [] : snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() })));
    });
    return () => unsubClients();
  }, []);
  ...
  const handleClientChange = (clientId) => {
    ...
    const sitesRef = collection(db, `clients/${clientId}/sites`);
    const unsubSites = onSnapshot(sitesRef, (sitesSnapshot) => {
      setSites(sitesSnapshot.empty ? [] : sitesSnapshot.docs.map((doc) => doc.id));
    });
  ...
  useEffect(() => {
    ...
    const rbtsRef = collection(db, `clients/${selectedClient}/sites/${selectedSite}/rbts`);
    const unsubRbts = onSnapshot(rbtsRef, (snapshot) => {
      setRbtList(snapshot.empty ? [] : snapshot.docs.map((doc) => ({ rbt_id: doc.id, ...doc.data() })));
      setLoading(false);
    });
